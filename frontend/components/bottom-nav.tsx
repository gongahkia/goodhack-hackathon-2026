...
import { CalendarDays, NotebookPen, Telescope } from "lucide-react";
import { clsx } from "clsx";
import { useI18n } from "@/lib/i18n";

const items = [
  { href: "/", labelKey: "nav.calendar", icon: CalendarDays },
  { href: "/capture", labelKey: "nav.capture", icon: NotebookPen },
...
export function BottomNav() {
  ...
  const { t } = useI18n();
  return (
    <nav className="sticky bottom-0 z-40 grid grid-cols-3 border-t border-[#dde5df] bg-white/95 px-2 py-2 backdrop-blur">
      {items.map((item) => {
